...
:LATEST_UPDATE:
  =Last Updated:= <2023-06-15 Thu> /(this template needs work)/
  ...
  - =PUBLIC USE Instructions:= /(FOR CLONES OF THIS TEMPLATE)/
     ...
     + [X] *COMPARE the TIMESTAMPS between THIS TEMPLATE and CLONES:*
     ...
     + [X] *Keep ABOVE timestamp within CLONES of this template UP TO DATE:*
...
=For Local Customizing:= EVERYTHING is pretty much done in this Setup File now...
Only the generic things common to all LaTeX export configurations are done in Emacs Lisp configurations... Only the names are added to the list so the dispatcher knows what to do... 
...
/(I may add a few more here as I find them, i.e., if I find them to be common hurdles/caveats for most folks)/
...
=LaTeX CONFIGURATIONS FOR: --> Short Excerpt PDF:=

=NOTE:= All Latex Configurations are done here for
      portability...  The ~excerpt~ class name itself
...
- *DISABLED LATEX DIRECTIVES:*
   ...
   #+LATEX_HEADER: \usepackage{lmodern}    %% Ensures we have correct font...
   ...
   #+LATEX_HEADER: \AtBeginDocument{\parindent = 0em \nonzeroparskip}
...
- *ENABLED LATEX DIRECTIVES:*
   ...
   =GENERAL PACKAGES - Options:=
    ...
    + *USE Better Source Code Listing Styles:*
  
       /Try:/ ~$> pygmentize -L styles~ /to see a list of all possible "minted" code
...
- *ENABLED ORG-MODE OPTIONS:*
    ...
    #+OPTIONS: num:nil
       # NO NUMBERING for SHORT PDF EXCERPTS...
       # (org-export-with-section-numbers)
       # When N, Number only headlines level N or above.
       # Set ‘UNNUMBERED’ property to non-nil to disable
       # numbering of single headings and subheadings.
       # Moreover, when value is ‘notoc’, the headline,
