...
:README:
   ...
   - *C-c C-x C-i* =Start Clock= /(Begin OR Resume Clocking Current task)/
                  This creates a new ~:CLOCKING:~ Time Card Entry on the
                  next line below the Org Heading you have chosen to
                  clock time for... Clocking Entries look like this:

         =Example:=

         ::CLOCKING:
           CLOCK: [2022-08-01 Mon 09:48]
         ::END:

         =Note:= /(I had to add an extra colon ~:~ char to the example below,/
                /otherwise org mode thinks it is real, but it is inside/
                /this/ ~:README:~ /which would cause a problem... Org-Mode/
                /does NOT allow nesting of Properties Boxes.../

         =Do above command now...= /Start recording your time girl!/
   ...
   Document your progress and important information related to the task
   here in the log as you work... =(be a lab rat! LOL)=
  ...
  - *C-c C-x C-o* =Stop the Clock= /(to stop clocking your time on task)/
                 ...
                 This *Finalizes the Time Card Entry* within ~:CLOCKING:~
                 /(created earlier when you started the clock)/...
                 The completed time card entry looks like this:

  The above entry shows start timestamp, end timestamp, and total time worked during this segment...  As you can see from the example above I left the clock running all day by accident.  If you do something like that you can always come back and directly edit the clock times by hand to amend them...  After changing clock times or dates, etc., Update the Time Card Entry with the standard "Do What this line expects you to do" key command:

  - *C-c C-c*

  The total time will change at the end showing your new calculated time...

  =Example:=

    ::CLOCKING:
       CLOCK: [2022-08-01 Mon 09:48]--[2022-08-01 Mon 09:56] =>  0:08
    ::END:
  ...
  - *C-c C-x C-q* =Cancel Current Clock=
                 ...
                 /(In Case You Accidentally Started It)/

   This removes currently running clock and forgets it...

  - *C-c C-x C-d* =Display Clocking Info=
     
                 /This will show elapsed times to the right of
                 headings.../

                 =WARNING!= This command will clutter up your org file
                          with time entries at the end of every heading!
