...
=These Instructions Were Last Modified:=
<2024-05-19 Sun>
...
=NOTE:= Currently there is no easy way to render fountain script directly within the PDF
             exported from this file using emacs normal PDF export commands... This template
             is only included here for the future if it ever does become a feature of emacs...

             For NOW you must create a stand alone ~.fountain script~ file and use the
             *afterwriting* python tool to produce a PDF... Then you can if needed use LaTeX
             directives within this file to include the external PDF (already rendered) within
             the final exported PDF of this .org file... /(or simply provide a link
     to the script PDF)/

             The Fountain Script below looks fine and proper for editing within the source
             code block below however... You can do this within org-mode to compose and
             later cut-paste your snippets into a stand alone .fountain script master file...
