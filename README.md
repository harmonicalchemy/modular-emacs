...
This commit represents **Modular Emacs: _Lisp IDE Fork:_ `Release V2.3.0` Q4-2019** which has it's own branch on Github (**`lisp-ide`**) with all of these new features turned on.  This fork has also been merged into the **Github `develop`** branch with all of the new features commented out.  That way I can develop and test these features but also keep most of the code in sync with the **`master`** and **`develop`** branches on-going _(with those features turned off within those branches of course...)_ 
